...
func TestAdjacentSegments(t *testing.T) {
	...
	fromSegmentNo, toSegmentNo, err := SegmentsAroundKnot(knots, 2, true, true)
	assert.Equal(t, 1, fromSegmentNo, "fromSegmentNo is 1")
	assert.Equal(t, 2, toSegmentNo, "toSegmentNo is 2")
	fromSegmentNo, toSegmentNo, err = SegmentsAroundKnot(knots, 2, true, false)
	assert.Equal(t, 1, fromSegmentNo, "fromSegmentNo is 1")
	assert.Equal(t, 1, toSegmentNo, "toSegmentNo is 1 (segment after is ignored)")
	_, _, err = SegmentsAroundKnot(knots, 0, true, false)
	assert.NotNil(t, err, "SegmentsAroundKnot don't exist, error must be not-nil")
	_, _, err = SegmentsAroundKnot(knots, knots.KnotCnt()-1, false, true)
	assert.NotNil(t, err, "SegmentsAroundKnot don't exist, error must be not-nil")
	_, _, err = SegmentsAroundKnot(knots, 2, false, false)
	assert.NotNil(t, err, "SegmentsAroundKnot don't exist, error must be not-nil")

	emptyKnots := NewUniformKnots(0)
	_, _, err = SegmentsAroundKnot(emptyKnots, 0, true, true)
	assert.NotNil(t, err, "KnotNo doesn't exist, error must be not-nil")

	singleKnots := NewUniformKnots(1)
	_, _, err = SegmentsAroundKnot(singleKnots, 0, true, true)
	assert.NotNil(t, err, "SegmentsAroundKnot don't exist, error must be not-nil")
}
